...
def generate(openapi_path: Path, out_path: Path):
    ...
    md.append("")
    md.extend(["## Endpoints", ""])  # header
    md.extend(render_table(paths, components))
    md.append("")
    md.append("For interactive docs visit <http://127.0.0.1:8080/docs> when the gateway is running.")
...
if __name__ == "__main__":
    ...
    if not openapi.exists():
        print("openapi.json not found. Run the gateway and fetch /openapi.json to this repo root as openapi.json.")
        raise SystemExit(1)
